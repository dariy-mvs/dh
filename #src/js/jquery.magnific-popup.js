 ...
 * Private functions
 */
  const _mfpOn = function( name, f ) {
    mfp.ev.on( NS + name + EVENT_NS, f );
  };
  const _getEl = function( className, appendTo, html, raw ) {
  ...
  const _mfpTrigger = function( e, data ) {
  ...
  const _getCloseBtn = function( type ) {
 ...
 */
  ...
  const _putInlineElementsBack = function() {
  ...
  const AJAX_NS = 'ajax';
  let _ajaxCur;
  const _removeAjaxCursor = function() {
    if ( _ajaxCur ) {
      $( document.body ).removeClass( _ajaxCur );
    }
  };
  const _destroyAjaxRequest = function() {
    _removeAjaxCursor();
    if ( mfp.req ) {
      mfp.req.abort();
    }
  ...
  /* >>zoom */
  let hasMozTransform;
  const getHasMozTransform = function() {
  ...
  $.magnificPopup.registerModule( 'zoom', {
    ...
    proto: {
      ...
      initZoom() {
        ...
        const getElToAnimate = function( image ) {
  ...
  const _fixIframeBugs = function( isShowing ) {
 ...
 */
  const _getLoopedId = function( index ) {
  ...
  const _replaceCurrTotal = function( text, curr, total ) {
